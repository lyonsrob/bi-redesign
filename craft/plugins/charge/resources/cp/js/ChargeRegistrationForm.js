(function($){
    ...
    if (typeof Craft.Charge === typeof undefined) {
        Craft.Charge = {};
    }
     ...
     */
    Craft.Charge.RegistrationForm = Craft.BaseElementIndex.extend({
        licenseKey: null,
        ...
        loadLicenseInfo: function(action) {
            Craft.postActionRequest('charge/license/getLicenseInfo', $.proxy(function(response, textStatus) {
                console.log('loading info');
                if (textStatus == 'success') {
                    this.unloadLoadingUi();
                    this.setLicenseKey(response.licenseKey);
                    console.log('getting response of : ');
        ...
        validateLicenseKey: function(licenseKey) {
            return (licenseKey.length == 29);
        },

        handleUnregisterLicenseFormSubmit: function(ev) {
            ev.preventDefault();
            this.$unregisterLicenseSpinner.removeClass('hidden');
            Craft.postActionRequest('charge/license/unregister', $.proxy(function(response, textStatus) {
        ...
        handleUpdateLicenseFormSubmit: function(ev) {
            ...
            Craft.postActionRequest('charge/license/updateLicenseKey', data, $.proxy(function(response, textStatus) {
        ...
        handleTransferLicenseFormSubmit: function(ev) {
            ...
            Craft.postActionRequest('charge/license/transfer', $.proxy(function(response, textStatus) {
